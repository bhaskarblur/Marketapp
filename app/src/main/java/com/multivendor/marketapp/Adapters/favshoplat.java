...
public class favshoplat extends RecyclerView.Adapter<favshoplat.viewHolder> {

    private Context mcontext;

    public favshoplat(Context mcontext, List<com.multivendor.marketapp.Models.nbyshopsModel> nbyshopsModel) {
    ...
    public viewHolder onCreateViewHolder( ViewGroup parent, int viewType) {
        ...
        View view= LayoutInflater.from(mcontext).inflate(R.layout.favshopslay,parent,false);
    ...
    public void onBindViewHolder( viewHolder holder, int position) {
        ...
        if(nbyshopsModel.get(position).getShopimg()!=null) {
            Picasso.get().load(nbyshopsModel.get(position).getShopimg()).transform(transformation)
                    .resize(150,150).into(holder.shopimg);
        }
        else {
            Picasso.get().load(R.drawable.imgsample).transform(transformation).fit().into(holder.shopimg);
        }
